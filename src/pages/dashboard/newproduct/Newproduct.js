...
const Newproduct = () => {
    ...
    const picproduct = async(e)=>{
        ...
        try{
            ...
            const {data} = 
                await axios.post('http://localhost:9000/api/uploade/multiple',formData,config)
    ...
    useEffect(()=>{
        setDetail([{width: width_Product,height: height_Product, length: length_Product}])
    ...
    const submitHandler  =(e)=>{
        ...
        if(!nameProduct) setErrorTitle(true); else setErrorTitle(false)
        
        if(!price) setErrorPrice(true);else  setErrorPrice(false)
        if (product_img === undefined || product_img === null || product_img === "") {
            setErrorPic(true);
          } else {
            ...
            dispatch(
                createProduct(
                    nameProduct,product_img,detail,price,count,describtionLive,categoryProduct,color
              )
            );
            resetHandler();
          }
        ...
        <div className={`col ${styles.content}`}>
            <div className={`row ${styles.container_content}`}  style={{    '--bs-gutter-x': '0rem'}}>
                <div className={`col ${styles.newContainer}`}>
                            ...
                            <Col xs={12}>
                                <div className={styles.form_0}>
                                    <Form.Group controlId='picproduct'>
                                        <Form.Label className={styles.form_label} >تصاویر محصولات</Form.Label>
                                        <Form.Control type='file' name="files"
                                        onChange={e=>setFiles(e.target.files)}
                                        className={` ${loade=== true ? 'disabled' : 'block-detail'}`}
                    ...
                    {/* detail product */}
                    <Form className={styles.formfix} onSubmit={submitHandler} dir='rtl' >
                        ...
                        {/* from2 -detail=>size */}
                            <h3 className={styles.title} dir="rtl">ویژگی محصول</h3>
                            <span className={styles.subtitle} dir="rtl">سایز</span>
                        ...
                        {/*  */}
                        {log}
                    {describtionLive === undefined | describtionLive === '' | describtionLive === null
